...
def main():
    ...
    (sock_family, sock_type, sock_proto, sock_canonname, sock_addr) = socket.getaddrinfo(host, 443, family=socket.AF_INET, proto=socket.IPPROTO_TCP)[0]
    ...
    with open('rs-com.html', 'w') as outfd:
        for line in response.decode().split('\r\n'):
            if (line.startswith(tuple(string.ascii_uppercase)) and ':' in line) or line.startswith('HTTP'):
                print(line)
            else:
                outfd.write(line)
